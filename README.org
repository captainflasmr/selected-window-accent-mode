...
By default, fringes are hidden in the =default= style.  If you rely on fringe indicators (line-wrapping arrows, git-gutter, flycheck, etc.) you can set =selected-window-accent-fringe-minimum= to a positive pixel value (e.g., 8) to keep fringes visible.  The fringe background will match the buffer background so indicators blend naturally.
...
To automatically sync the tab bar whenever you switch themes, add the following to your configuration:
